...
Phase 4   录屏 + 后期
   ▼
Phase 5   Skill 自我提升（可选但推荐）
          用本次项目的验收反馈 / 失败修复 / 录屏结果沉淀经验；
          满足条件时离线跑 eval / GEPA 风格优化，不在用户项目中临时改规则。
```
...
| Phase 5 Skill 自我提升 | `references/SELF-IMPROVEMENT.md` | 本次项目的 `script.md` / `outline.md` / 章节代码 / 用户反馈 / 自检失败项 |
...
详细流程见 [`references/SELF-IMPROVEMENT.md`](references/SELF-IMPROVEMENT.md)。
本阶段分两层做：

- **在线沉淀**：每次完整使用本 Skill 后，把用户反馈、失败修复、验收卡点、
  自检 fail 项抽成经验；优先补到现有章节/主题/音频/录屏指引，不为单次项目
  新造窄规则。
- **离线优化**：当同类问题重复出现，或用户明确要求“让这个 skill 自我提升”
  时，把经验做成 eval case，用“基线规则 vs 候选改写”对比，通过约束门后再
  修改 Skill 文件。

**触发条件**（任一满足就执行 Phase 5）：用户要求改进本 Skill；同一类问题在
本项目中出现 2 次以上；章节验收暴露 `CHAPTER-CRAFT.md` 没覆盖的盲区；音频
/ Auto 录屏错位；主题 token / 脚手架约束导致章节反复返工。

**落地规则**：

- 小修直接 patch 目标指引，并在改动处附近写短注释说明来源。
- 大修先写 eval case，再比较候选方案；没有通过验证，不把候选合入主 Skill。
- 自我提升只改本 Skill 的 `SKILL.md` / `references/` / `scripts/` / `templates/`
  / `themes/`，不把某个用户项目里的临时实现搬成通用规则。
- 改完后必须汇报：经验来源、改了哪些文件、用什么 case 验证、剩余风险。
...
| [`references/SELF-IMPROVEMENT.md`](references/SELF-IMPROVEMENT.md) | Phase 5 才读 | 用后经验沉淀、离线 eval / GEPA 风格优化、约束门 |
